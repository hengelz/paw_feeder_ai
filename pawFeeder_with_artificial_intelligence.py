...
def initializeCloud():
    try:
        ...
        setDBVariableValue('hasFood', False)
        setDBVariableValue('imageLabels', '')
...
def takePhoto():
    try:
        ...
        print('Error while taking the picture, please check connection')
...
def analyzePhoto():
    imageDescription = image_analyzer.getImageDescription()
    hasFood = image_analyzer.checkForFood(imageDescription)
    imageLabels = image_analyzer.getPhotoLabels(imageDescription)
    setDBVariableValue('hasFood', hasFood)
    setDBVariableValue('imageLabels', imageLabels)
    
def main():
    print('Starting Pet feeder')
    print('Is AI on?:%s' % useAI)
    global webButtonToRunFeeder
    global webPhotoButton
    initializeCloud()
    turnOffMotor()
